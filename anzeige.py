    ...
    def buildList(self,listnew,d1):     
        ...
        for x in listnew:
             ...
             for xa in x[0]:
                ...
                if xa==x[0][-1]:Farben().farb_re(prog_colors[4])#color=int(prog_colors[4].lstrip('#'), 16)
                if xa[2]==heute:
            ...
            for i in range(5):
                ...
                for x in range(7):
                    ...
                    if sch_tag+1>len(turnus):
                        sch_tag=sch_tag % len(turnus)
                    if not wo:wo=dat.isocalendar()[1]
                    if dat==heute:
                        d1=dat.day
        ...
        for x in listnew:
            ...
            if x[2] =="comment":
	         res.append((eListboxPythonMultiContent.TYPE_TEXT,0, 5, 450, 30, 0, RT_HALIGN_CENTER | RT_VALIGN_CENTER, x[0],int(prog_colors[3], 16),int(prog_colors[3], 16)))
                ...
                if x[1]:bgcol= Farben().farb_re(x[1])#int(x[1].lstrip('#'), 16)
...
class color_liste(MenuList): 
    ...
    def buildList(self,listnew,s_color):
        list=[]
        if s_color:s_color=int(s_color.lstrip('#'), 16)
        i=0
        sel_index=0
        for x in listnew:
            res = [ x ]
